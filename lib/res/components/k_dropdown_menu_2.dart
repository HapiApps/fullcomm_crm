...
class MyDropdownMenu2<T> extends StatefulWidget {
  const MyDropdownMenu2({
    ...
    this.inputFormatters, this.labelText, this.highlightedIndex, this.hintstyle, this.labelstyle,
  }) : assert(filterCallback == null || enableFilter);

  final bool enabled;
  /// External selected item (preferred over initialSelection when provided)
...
class _MyDropdownMenuState<T> extends State<MyDropdownMenu2<T>> {
  ...
  MyDropdownMenuEntry2<T>? get highlightedEntry {
    if (currentHighlight == null) return null;
    if (currentHighlight! < 0 || currentHighlight! >= filteredEntries.length) return null;
    return filteredEntries[currentHighlight!];
  }
  ...
  T? get highlightedValue => highlightedEntry?.value;
  /// Public method to select the currently highlighted item (useful for ENTER)

  void selectHighlightedItem() {
    if (!mounted) return;

    // If there is a highlight, select it from filteredEntries
    if (currentHighlight != null &&
        currentHighlight! >= 0 &&
        currentHighlight! < filteredEntries.length) {
      ...
      // After selection, close menu and reset highlight (if search enabled)
      setState(() {
        if (widget.enableSearch) {
          // keep highlight as selected index within full list mapping if needed
    ...
    // If there's no highlight, attempt to match typed text to an entry label exactly
    final typed = _localTextEditingController?.text.trim() ?? '';
    if (typed.isNotEmpty) {
      final index = filteredEntries.indexWhere((e) => e.label == typed);
      if (index != -1) {
  ...
  TextEditingValue get _initialTextEditingValue {
    ...
    for (final MyDropdownMenuEntry2<T> entry in widget.dropdownMenuEntries) {
  ...
  void didUpdateWidget(MyDropdownMenu2<T> oldWidget) {
    ...
    if (oldWidget.controller != widget.controller) {
      if (oldWidget.controller == null) {
        // dispose old local only if it was created locally
        _localTextEditingController?.dispose();
      }
      _localTextEditingController = widget.controller ?? TextEditingController();
      // update displayed text for new controller
      _localTextEditingController?.value = _initialTextEditingValue;
    }
    ...
    // react to selectedItem changes
    if (oldWidget.selectedItem != widget.selectedItem || oldWidget.initialSelection != widget.initialSelection) {
      _localTextEditingController?.value = _initialTextEditingValue;
    }
  }
  ...
  List<Widget> _buildButtons(
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      ...
      // ⭐ Use index to fake focused state for keyboard highlight
      final bool isFocused = (focusedIndex == i);

      // ⭐ Orange color for hover, focused, pressed
      ...
      result.add(
        MenuItemButton(
          ...
          // ⭐ Important: Do NOT use selected:, it's unsupported
  ...
  void handleUpKeyInvoke(_) {
    setState(() {
      ...
      if (currentHighlight == null) {
        currentHighlight = filteredEntries.length - 1;
      } else {
        currentHighlight = (currentHighlight! - 1 + filteredEntries.length) % filteredEntries.length;
      }
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              final text = _localTextEditingController?.text.trim() ?? '';

              // ⭐ Custom ENTER behavior (your popup)
              ...
              if (currentHighlight != null &&
                ...
                // If no highlight, try to match typed exact label
                final matchIndex = filteredEntries.indexWhere((e) => e.label == text);
                if (matchIndex != -1) {
                  final entry = filteredEntries[matchIndex];
                  _localTextEditingController?.value = TextEditingValue(
                    text: entry.label,
                    selection: TextSelection.collapsed(offset: entry.label.length),
                  );
