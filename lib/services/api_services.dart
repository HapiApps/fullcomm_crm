...
class ApiService {
  ...
  Future insertEmployeeApi(BuildContext context) async {
    try{
      ...
      request.fields["email"]=stringToBase64.encode(controllers.empEmail.value);
      request.fields["phone_number"]=controllers.empPhone.value;
      request.fields["dob"]=controllers.empDOB.value;
      request.fields["department"]=stringToBase64.encode(controllers.department);
      ...
      request.fields["manager_name"]=stringToBase64.encode(controllers.empManagerName.value);
      request.fields["salary"]=stringToBase64.encode(controllers.empSalary.value);
      request.fields["bonus"]=stringToBase64.encode(controllers.empBonus.value);
      request.fields["benefits"]=stringToBase64.encode(controllers.benefits);
      request.fields["degree"]=stringToBase64.encode(controllers.empDegree.value);
      request.fields["institution"]=stringToBase64.encode(controllers.empInstitution.value);
      request.fields["graduation_year"]=stringToBase64.encode(controllers.empGraYear.value);
      request.fields["gender"]=stringToBase64.encode(controllers.selectedGender);
      request.fields["marital_status"]=stringToBase64.encode(controllers.selectedMarital);
      request.fields["relation_ship"]=stringToBase64.encode(controllers.selectedRelationShip);
      request.fields["cos_id"]=cosId;
      request.fields["door_no"]=stringToBase64.encode(controllers.empDoorNo.value);
      request.fields["street_name"]=stringToBase64.encode(controllers.empStreet.value);
      ...
      request.fields["country"]=stringToBase64.encode(controllers.empCountry.value);
      request.fields["pin_code"]=stringToBase64.encode(controllers.empPinCode.value);
      ...
      if(response.statusCode==200&& output.body=="{\"status_code\":200,\"message\":\"ok\"}"){
        print("success");
  ...
  Future insertLeadAPI(BuildContext context) async {
    ...
    try{
      ...
      Map data ={
        ...
        "budget":controllers.budgetCrt.text.isEmpty?"0":controllers.budgetCrt.text.trim(),
      ...
      if (request.statusCode == 200 && response["message"]=="ok"){
        utils.snackBar(msg: "Your Lead is created successfully !",
  ...
  Future insertProductAPI(BuildContext context) async {
    try{
      ...
      final request = await http.post(Uri.parse(insertProduct),
          // headers: {
          //   "Accept": "application/text",
          //   "Content-Type": "application/x-www-form-urlencoded"
          // },
      ...
      if (request.statusCode == 200){
        utils.snackBar(msg: "Your product created successfully",color: colorsConst.primary,context:Get.context!);
        //final prefs =await SharedPreferences.getInstance();
  ...
  Future insertProspectsAPI(BuildContext context,List<Map<String, String>> list) async {
    try{
  ...
  Future insertCustomersAPI(BuildContext context, List<Map<String, dynamic>> customerData) async {
    try {
  ...
  Future<void> insertSingleCustomer(context) async {
    try {
      ...
      //   }
      // }
      leadId = controllers.leadCategory=="Suspects"?"1":controllers.leadCategory=="Prospects"?"2":controllers.leadCategory=="Qualified"?"3":"4";
      var callListId;
      for (var role in controllers.callList) {
        if (role['value'] == controllers.visitType) {
          callListId = role['id'];
     ...
     // }
      ...
      Map<String, dynamic> data = {
        ...
        "visit_type": callListId,
        "data": jsonString,
        'prospect_enrollment_date' : controllers.prospectEnrollmentDateCrt.text.trim(),
        'expected_convertion_date' : controllers.expectedConversionDateCrt.text.trim(),
  ...
  Future insertCompanyAPI(BuildContext context) async {
    try{
      ...
      print("data ${data.toString()}");
      final request = await http.post(Uri.parse(insertCompany),
          // headers: {
          //   "Accept": "application/text",
          //   "Content-Type": "application/x-www-form-urlencoded"
          // },
      ...
      print("request ${request.body}");
      // Map<String, dynamic> response = json.decode(request.body);
      if (request.statusCode == 200 && request.body=="{\"status_code\":200,\"message\":\"ok\"}"){
        utils.snackBar(msg: "Your company created successfully",color: colorsConst.primary,context:Get.context!);
        final prefs =await SharedPreferences.getInstance();
        prefs.remove("coName");
  ...
  Future getVisitType() async {
    try{
      ...
      if (request.statusCode == 200){
        ...
        print("call data ${controllers.callNameList}");
  ...
  Future insertUsersPHP(BuildContext context) async {
    try{
      ...
      if (request.statusCode == 200){
        utils.snackBar(context: Get.context!, msg: "User created successfully",color:Colors.green);
        ...
        errorDialog(Get.context!,"Employee with this Phone Number already exits");
          ...
          return AlertDialog(
            ...
            icon: Align(
                ...
                child: IconButton(
                    ...
                    icon: Icon(Icons.close,color: colorsConst.third,))),
            title: const Center(
  ...
  Future<List<ProductObj>> allProductDetails() async{
    ...
    try {
      ...
      throw Exception('Server error product: ${e.toString()}'); // Handle HTTP errors
    } catch (e) {
      throw Exception('Unexpected error product: ${e.toString()}'); // Catch other exceptions
    }
  }
  ...
  Future<List<CommentsObj>> allCommentDetails(String id) async{
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        controllers.allDirectVisit.value ="0";
        controllers.allTelephoneCalls.value = "0";
        throw Exception('Failed to load products: Status code ${response.body}');
      }
    } on SocketException {
      controllers.allDirectVisit.value ="0";
      controllers.allTelephoneCalls.value = "0";
 ...
 void mailReceiveDetails(String id)async{
   ...
   try {
     ...
     if (response.statusCode == 200){
       ...
       controllers.emailCount.value=int.parse(controllers.mailReceivesList[controllers.mailReceivesList.length-1]['sent_count']);
       print(controllers.emailCount.value);
       //print("Mails ${controllers.mailReceivesList}");
     } else {
       controllers.mailReceivesList.value=[];
       throw Exception('Failed to load leads: Status code ${response.statusCode}'); // Provide more specific error message
     }
   } on SocketException {
     controllers.mailReceivesList.value=[];
  ...
  Future<List<EmployeeObj>> allEmployeeDetails() async{
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load employee: Status code ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('No internet connection');
    } on HttpException catch (e) {
