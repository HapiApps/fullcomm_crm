...
class _CallCommentsState extends State<CallComments> {
  bool dateCheck(){
    return controllers.stDate.value!="${DateTime.now().day.toString().padLeft(2,"0")}"
              ...
              child: Column(
                ...
                children: [
                 ...
                 10.height,
                          ...
                          children:[
                            IconButton(
                                ...
                                onPressed:(){
                                  ...
                                  var data=DateTime.parse("${controllers.stDate.value.split("-").last}-${controllers.stDate.value.split("-")[1]}-${controllers.stDate.value.split("-").first}").subtract(const Duration(days: 1));
                                  controllers.stDate.value="${data.day.toString().padLeft(2,"0")}"
                            ...
                            // ),
                             Icon(Icons.calendar_today_outlined,size: 13,color: colorsConst.textColor,),
                           InkWell(
                              onTap: (){
                                if(context.mounted){
                                  ...
                                  showDatePicker(
                                    ...
                                    controllers.isCommentsLoading.value=false;
                                    controllers.stDate.value="${controllers.dateTime.day.toString().padLeft(2,"0")}"
                            ...
                            Obx(()=>IconButton(
                                onPressed: (){
                                  if(dateCheck()){
                                    ...
                                    // controllers.isAllContacts.value=true;
                                     controllers.isCommentsLoading.value=false;
                                    var data=DateTime.parse("${controllers.stDate.value.split("-").last}-${controllers.stDate.value.split("-")[1]}-${controllers.stDate.value.split("-").first}").add(const Duration(days: 1));
                                    controllers.stDate.value="${data.day.toString().padLeft(2,"0")}"
                                        "-${data.month.toString().padLeft(2,"0")}"
                                        "-${data.year.toString()}";
                                   controllers.isCommentsLoading.value=true;
                              ...
                              size: 16,
                              color:dateCheck()?colorsConst.third:Colors.grey,)),
                            ),
                            //)
                          ],
                        ),
                  ...
                  8.height,
                Obx(()=>  controllers.isCommentsLoading.value==false?
                Center(child:CircularProgressIndicator(
                  ...
                  child:FutureBuilder<List<CommentsObj>>(
                    ...
                    builder:(context, snapshot){
                      if(snapshot.hasData){
                        DateTime startDate = DateFormat('dd-MM-yyyy').parse(controllers.stDate.value);
                        ...
                        //   }
                        // }
                        if(controllers.stDate.value=="${currentDate.day.toString().padLeft(2,"0")}"
                            "-${currentDate.month.toString().padLeft(2,"0")}"
                            "-${currentDate.year.toString()}"){
                          filteredComments = filteredComments;
                        }else{
                          filteredComments = filteredComments.where((contact){
                            DateTime contactDate = DateFormat('yyyy-MM-dd').parse(contact.updateTs.toString().split(' ')[0]);
                            return contactDate.isAfter(startDate) && contactDate.isBefore(currentDate) && _isContactMatchingSearch(contact, searchText);
                          }).toList();
                        }

                        // if(filteredComments.isNotEmpty){
                           filteredComments = filteredComments.where((contact){
                             return _isContactMatchingSearch(contact, searchText);
                           }).toList();
                         //}
                        ...
                        return ListView.separated(
                            ...
                            itemBuilder:(context, index){
                              ...
                              String inputDate = data.createdTs.toString();
                              DateTime parsedDate = DateTime.parse(inputDate);

                              String formattedDate = DateFormat("MMM d, yyyy  hh:mm a").format(parsedDate.toLocal());
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            CircleAvatar(
                                              ...
                                              radius: 13,
                                              child: Icon(data.type=="1"?Icons.person_outline:Icons.call,color: colorsConst.primary,size: 13,),
                                            ),
                                            60.height,
                                            8.width,
                                            Container(
