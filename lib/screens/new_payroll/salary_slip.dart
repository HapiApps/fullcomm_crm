...
class _SalarySlipState extends State<SalarySlip> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: PreferredSize(preferredSize: const Size(300, 70),
            child: app_bar(text: "Salary Slip", callback1: () {
                      ...
                      showMonthPicker(
                        ...
                        function: (){
                          if(pyrlCtr.unitName!=null){
                            newPyrlServ.getSalarySlip(unitId);
                          }
                        }
                      );
                    },
  ...
  static Future<void> generatePayrollPdf(RxList<UnitPayroll> unitPayrollList) async {
    ...
    if (kIsWeb) {
      await Printing.sharePdf(
        bytes: await pdf.save(),
        filename: '${unitPayrollList[0].unitName} ${pyrlCtr.month.value} Salary Slip.pdf',
      );
    } else {
      final dir = await getApplicationDocumentsDirectory();
      final file = File('${dir.path}/${unitPayrollList[0].unitName} ${pyrlCtr.month.value} Salary Slip.pdf');
      await file.writeAsBytes(await pdf.save());
    }
  }
