...
class _EditProductDialogState extends State<EditProductDialog> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // p.category.text=widget.category;
      // p.subCategory.text=widget.subCategory;
      p.units.text=widget.units;
      p.isLoose=widget.isLoose;
      p.variations.text=widget.variations;
      p.mrp.text=widget.mrp;
      p.outPrice.text=widget.outPrice;
      p.inPrice.text=widget.inPrice;

      final category = p.categories.firstWhereOrNull(
            (c) => c.title == widget.category,
      );

      if (category != null) {
        p.selectedCategoryId = category.id;

        final subCategory = category.subcategories.firstWhereOrNull(
              (s) => s.title == widget.subCategory,
        );

        if (subCategory != null) {
          p.selectedSubCategoryId = subCategory.id;
        }
      }
      if (p.categories.isEmpty) p.fetchCategories();
      nameFN.requestFocus();
  ...
  void _save(BillingProvider p) {
    ...
    if (p.loading) return;
    if (!p.validatePrices(p, context)) return;
    p.updateProduct(context,widget.id);
  }
